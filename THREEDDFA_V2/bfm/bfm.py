...
import folder_paths
real_path=osp.join(folder_paths.base_path,"custom_nodes/ComfyUI_SHMT/THREEDDFA_V2")
#make_abs_path = lambda fn: osp.join(real_path, fn)
...
class BFMModel(object):
    def __init__(self, bfm_fp, shape_dim=40, exp_dim=10):
        ...
        if osp.split(bfm_fp)[-1] == 'bfm_noneck_v3.pkl':
            self.tri = _load(osp.join(real_path,'configs/tri.pkl'))  # this tri/face is re-built for bfm_noneck_v3
        else:
